...
bool view_main_create(Evas_Object *parent)
{
	...
	dlog_print(DLOG_INFO, LOG_TAG, "Creating Main view");
	/*char *edj_path = NULL;
	...
	edj_path = data_create_resource_path(EDJ_MAIN); //TODO: load edje file
	...
	s_info.layout = elm_layout_add(parent);
	Evas_Object *label;
	label = elm_label_add(s_info.layout);
	elm_object_text_set(label, "<align=center>Hello Tizenbingbong</align>");
	evas_object_size_hint_weight_set(label, 1, 1);
	elm_object_content_set(s_info.layout, label);

	Evas_Object *testhingy;
	testhingy = elm_button_add(s_info.layout);

	elm_object_content_set(s_info.layout, testhingy);
